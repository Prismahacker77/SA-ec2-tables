...
def get_gateway_for_subnet(ec2, subnet_id, vpc_id):
    route_tables = ec2.describe_route_tables(Filters=[{'Name': 'vpc-id', 'Values': [vpc_id]}])['RouteTables']
    for rt in route_tables:
        for route in rt['Routes']:
            if route.get('DestinationCidrBlock') == '0.0.0.0/0':
                for assoc in rt['Associations']:
                    if assoc.get('SubnetId') == subnet_id:
                        return route.get('GatewayId', route.get('NatGatewayId', 'None'))
    return 'None'
...
def scan_ec2_instances():
    regions = get_regions()
    table = PrettyTable()
    table.field_names = ["Region", "EC2 Count", "Instance ID", "Internet Accessible", "Public IP", "Subnet ID", "Gateway", "VPC ID", "Availability Zone"]
    ...
    for region in regions:
        ...
        for reservation in instances:
            for instance in reservation['Instances']:
                ...
                gateway = get_gateway_for_subnet(ec2, subnet_id, vpc_id)
                internet_accessible = "No"

                if vpc_id and subnet_id:
                    internet_accessible = is_internet_accessible(ec2, subnet_id, vpc_id)

                table.add_row([region, ec2_count, instance_id, internet_accessible, public_ip, subnet_id, gateway, vpc_id, az])
